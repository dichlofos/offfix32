...
int main(int argc, char** argv)
{
...
#endif
	if (!fI) Shutdown("*** cannot open input file!", fI, fO);
	...
	// check header 
	if (!CheckRIFFHeader(nRIFFHeader)) Shutdown("*** RIFF header check failed!", fI, fO);
...
#endif
	if (!fO) Shutdown("*** cannot open output file!", fI, fO);
	...
	if (fwrite(nRIFFHeader, 4, 11, fO) != 11) Shutdown("*** cannot write RIFF header to output file!", fI, fO);
	...
	if (nOffsetValue < 0)
	{
		...
		while (i < nSampleCount - nOffsetValue)
		{
			...
			if (fread(chBuffer, 4, nCurrentBlockSize, fI) != nCurrentBlockSize) Shutdown("*** cannot read from input file!", fI, fO);
			if (fwrite(chBuffer, 4, nCurrentBlockSize, fO) != nCurrentBlockSize) Shutdown("*** cannot write to output file!", fI, fO);
